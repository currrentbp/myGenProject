...
所有左子树和右子树自身必须也是二叉搜索树。
    ...
    public boolean isValidBST(TreeNode root) {
        ...
        for (int i = 0; i < treeNodes.size(); i++) {
            TreeNode currentTreeNode = treeNodes.get(i);
            if (currentTreeNode == null) {
                continue;
            }
            TreeNode left = currentTreeNode.left;
            TreeNode right = currentTreeNode.right;
            if (null != left && left.val > currentTreeNode.val) {
                return false;
            }
            if (null != right && right.val < currentTreeNode.val) {
                return false;
            }
            if (null != left) {
                treeNodes.add(left);
